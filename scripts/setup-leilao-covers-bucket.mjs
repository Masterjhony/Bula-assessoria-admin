// Cria (se não existir) o bucket `leilao-covers` no Supabase Storage e
// aplica as policies da migration 0014. Idempotente: pode rodar várias vezes.
//
// Uso: node scripts/setup-leilao-covers-bucket.mjs
...
if (exists) {
  ...
  console.log(`Bucket "${BUCKET}" criado (público, até 10MB, imagens).`)
}

console.log('\nLembrete: rode também a migration 0014 para criar as policies:')
console.log('  node scripts/apply-migration-single.mjs 0014_leilao_covers_bucket.sql')
