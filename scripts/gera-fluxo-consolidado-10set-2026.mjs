/**
 * Fluxo de caixa CONSOLIDADO 20/08 -> 10/09/2026.
 * Uma linha de caixa so: Sicoob + Sicredi somados. Nao ha leitura por banco.
 *
 * Pergunta: o caixa comporta pagar as comissoes de julho (25/08) e, depois, a folha
 * de agosto (05/09), contando com as parcelas que ainda entram de outros leiloes?
 *
 * Ancoras: Sicoob 29.169,19 (extrato de 20/08 13:47, conciliado 1:1) + Sicredi 12.598,64
 * (posicao do ERP em 03-04/08; o extrato de agosto daquele banco nunca foi importado).
...
D.caixa.inicial = r2(D.caixa.sicoob + D.caixa.sicrediLiquido)
D.caixa.saldoExtrato = 29169.19
D.caixa.confere = Math.abs(D.caixa.sicoob - D.caixa.saldoExtrato) < 0.005
D.caixa.composicao = [
  { rot: 'Sicoob — conta corrente 1.056-1', valor: D.caixa.sicoob, nota: 'conciliado 1:1 com o extrato de hoje, 13h47' },
  { rot: 'Sicredi — aplicação 53609-7', valor: D.caixa.sicrediApp, nota: 'posição do ERP em 03/08; extrato de agosto não importado' },
]
...
D.necessidade = [
  { rot: 'Só não ficar negativo', colchao: 0, linha: necessidade(0, {}) },
  { rot: 'Manter colchão de R$ 20.000', colchao: 20000, linha: necessidade(20000, {}) },
  { rot: 'Manter colchão de R$ 30.000', colchao: 30000, linha: necessidade(30000, {}) },
  { rot: 'Sem colchão, adiando as comissões para 05/09', colchao: 0, linha: necessidade(0, { adiaComissoes: '2026-09-05' }) },
]
...
D.capacidade = [
  ...
  { rot: 'Colchão de R$ 20.000', colchao: 20000 },
...
console.log('CAIXA CONSOLIDADO ', f2(D.caixa.inicial), '= Sicoob', f2(D.caixa.sicoob), D.caixa.confere ? '(bate)' : '(NAO BATE!)', '+ Sicredi', f2(D.caixa.sicrediLiquido))
...
console.log('')
console.log('SICREDI  ultimo movimento', D.sicredi.ultimoMovimentoCC, '(' + D.sicredi.diasSemExtrato + 'd atras) — extrato de agosto nao importado')
console.log('')
console.log('CENARIOS  (24/08 | 25/08 | 04/09 | 05/09 | 10/09 | minimo | dias neg)')
D.cenarios.forEach(c => console.log('  ' + c.chave.padEnd(8), f2(c.antesComissao).padStart(12), f2(c.saldo25ago).padStart(12), f2(c.antesFolha).padStart(12), f2(c.saldo05set).padStart(12), f2(c.saldo10set).padStart(12), f2(c.minimo.saldo).padStart(12), c.minimo.data, String(c.diasNegativos).padStart(3), c.pagaComissao ? 'com OK' : '', c.pagaFolha ? 'folha OK' : ''))
console.log('')
...
D.capacidade.forEach(c => console.log('   ' + c.rot.padEnd(38), f2(c.valor).padStart(12), '(' + c.pct + '%)', '| com EAO', f2(c.valorEao).padStart(12), '(' + c.pctEao + '%)'))
console.log('')
