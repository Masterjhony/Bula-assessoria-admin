// Cria (se não existir) o bucket `leilao-catalogos` no Supabase Storage e
// aplica as policies da migration 0018. Idempotente: pode rodar várias vezes.
//
// Uso: node scripts/setup-leilao-catalogos-bucket.mjs
...
if (exists) {
  ...
  console.log(`Bucket "${BUCKET}" criado (público, até 25MB, PDF).`)
}

console.log('\nLembrete: rode também a migration 0018 para criar as policies:')
console.log('  node scripts/apply-migration-single.mjs 0018_leilao_catalogos_bucket.sql')
