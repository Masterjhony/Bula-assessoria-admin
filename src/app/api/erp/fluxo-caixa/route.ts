...
export async function GET(req: NextRequest) {
  ...
  const [bancos, mov, cp, cr, catsTransf] = await Promise.all([
    sb.from('erp_contas_bancarias').select('saldo_atual').eq('ativo', true),
    sb.from('erp_movimentos_bancarios').select('data,tipo,valor,categoria_id').gte('data', inicioIso).lte('data', hoje.toISOString().slice(0, 10)),
    // sem gte(hoje): titulo VENCIDO e nao pago continua sendo caixa futuro — cai no bucket de hoje
    sb.from('erp_contas_pagar').select('vencimento,valor,desconto,juros,multa,valor_pago').in('status', ['aberto', 'parcial', 'vencido']).lte('vencimento', fimIso),
    sb.from('erp_contas_receber').select('vencimento,valor,desconto,juros,multa,valor_recebido').in('status', ['aberto', 'parcial', 'vencido']).lte('vencimento', fimIso),
    sb.from('erp_categorias').select('id').ilike('nome', 'Transferencias Internas%'),
  ])
  const transfIds = new Set(((catsTransf.data || []) as { id: string }[]).map((c) => c.id))
  ...
  for (const m of (mov.data || []) as { data: string; tipo: string; valor: number; categoria_id: string | null }[]) {
    const k = m.data; if (!dataMap[k]) continue
    if (m.categoria_id && transfIds.has(m.categoria_id)) continue // transferencia entre contas proprias nao e fluxo
  ...
  for (const r of (cp.data || []) as { vencimento: string; valor: number; desconto: number; juros: number; multa: number; valor_pago: number }[]) {
    const k = r.vencimento < hojeIso ? hojeIso : r.vencimento // vencido -> bucket de hoje
  ...
  // Calcular saldo projetado dia a dia (de hoje em diante, soma previstos;
  // hoje incluso: titulos vencidos/do dia ainda por liquidar afetam a projecao)
